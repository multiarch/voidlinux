...
for version in "${versions[@]}"; do
    ...
    voidlinux_version="$(get_part "$dir" version)"
	...
	if [ ! -f iso-slim/rootfs.tar ]; then
	    virt-tar-out -a "${isofilename}" / - > iso-slim/rootfs.tar.tmp
	    mv iso-slim/rootfs.tar.tmp iso-slim/rootfs.tar
	fi
    fi
...
FROM scratch
ADD rootfs.tar /
ENV ARCH=${scw_arch} VOIDLINUX_VERSION=${voidlinux_version} DOCKER_REPO=${repo} VOIDLINUX_IMAGE_URL=${url} QEMU_ARCH=${qemu_arch}
...
RUN yum remove -y \
      kernel-* *-firmware grub* voidlinux-logos mariadb*     \
...
EOF
	...
	docker import \
	       -c "ENV ARCH=${scw_arch} VOIDLINUX_VERSION=${voidlinux_version} DOCKER_REPO=${repo} VOIDLINUX_IMAGE_URL=${url} QEMU_ARCH=${qemu_arch}" \
	       - "$repo:$version-clean"
    docker rm "$tmpname"
    for tag in $tags; do
